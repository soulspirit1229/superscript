...
class SuperScript {
  ...
  getUser(userId, callback) {
    this.chatSystem.User.findOne({ id: userId }).slice('history', 10).exec(callback);
  }
  ...
  findOrCreateUser(userId, callback) {
    this.chatSystem.User.findOneAndUpdate({ id: userId }, {}, {
      ...
      new: true
    }).slice('history', 10).exec(callback);
  }
...
class SuperScriptInstance {
  ...
  getBot(tenantId) {
    return new SuperScript(this.coreChatSystem, this.coreFactSystem, this.plugins, this.scope, this.editMode, this.conversationTimeout, tenantId);
  }
}
